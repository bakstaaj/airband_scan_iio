 ...
 *   gcc -O2 -Wall -Wextra -o airband_scan pluto_airband_scan_v10.c -liio -lm
 *
 ...
 *
 * v9 note: clears the terminal before summary refreshes and changes the
 * default summary table size to 15 rows.
 *
 * v10 note: default console output is the refreshed summary table only,
 * event lines are opt-in with --events or --json, and CHIRP export defaults
 * to chirp.csv unless disabled with --no-chirp.
 *
 * Example:
...
#define DEFAULT_SUMMARY_TOP       15
#define DEFAULT_SUMMARY_EVERY_MS  5000LL
#define DEFAULT_CHIRP_CSV_PATH    "chirp.csv"
...
static void modesInitConfig(void) {
    ...
    Modes.summary_every_ms = DEFAULT_SUMMARY_EVERY_MS;
    Modes.summary_top = DEFAULT_SUMMARY_TOP;
    Modes.clear_summary_screen = 1;
    Modes.last_summary_ms = 0;

    Modes.event_output = 0;
    Modes.json_output = 0;
    Modes.csv_log_path = NULL;
    Modes.csv_log = NULL;

    Modes.chirp_csv_path = DEFAULT_CHIRP_CSV_PATH;
...
static void emitEvent(const char *event, const struct air_channel *ch,
    ...
    if (Modes.json_output) {
        ...
        fflush(stdout);
    } else if (Modes.event_output) {
        if (!strcmp(event, "ACTIVE")) {
            if (ch->label[0]) {
...
static void printSummary(long long now_ms, int force) {
    ...
    formatTimeMs(now_ms, tbuf, sizeof(tbuf));
    if (Modes.clear_summary_screen && isatty(STDERR_FILENO)) {
        /* Clear screen and move cursor home before each summary refresh. */
...
static void showHelp(void) {
    printf(
        ...
        "\n"
        "Summary and logging:\n"
        "  --summary-every <sec>    Print top activity table every N seconds, default 5\n"
        "  --summary-top <N>        Rows in summary table, default 15\n"
        "  --no-summary             Disable the default summary table\n"
        "  --no-summary-clear       Do not clear terminal before summary refresh\n"
        "  --events                 Print ACTIVE/ENDED event lines as well as the summary table\n"
        "  --log <file.csv>         Append ACTIVE/ENDED events to CSV\n"
        "  --json                   Print ACTIVE/ENDED events as JSON lines to stdout\n"
        "  --chirp <file.csv>       CHIRP Generic CSV output, default chirp.csv\n"
        "  --no-chirp               Disable default CHIRP CSV output\n"
        ...
        "\n"
        "Example:\n"
        "  ./airband_scan --freq-file phoenix_airband.txt --priority\n"
        "  ./airband_scan --freq-file phoenix_airband.txt --events --log airband.csv\n"
...
int main(int argc, char **argv) {
    ...
    for (int j = 1; j < argc; j++) {
        ...
        } else if (!strcmp(argv[j], "--no-summary")) {
            Modes.summary_every_ms = 0;
        } else if (!strcmp(argv[j], "--no-summary-clear")) {
            Modes.clear_summary_screen = 0;
        } else if (!strcmp(argv[j], "--events")) {
            Modes.event_output = 1;
        ...
        } else if (!strcmp(argv[j], "--no-chirp")) {
    ...
    if (Modes.verbose || Modes.debug_iio) {
        ...
        if (Modes.chirp_csv_path) {
            fprintf(stderr, "CHIRP export file: %s\n", Modes.chirp_csv_path);
        }
    }
