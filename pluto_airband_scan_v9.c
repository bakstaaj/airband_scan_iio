 ...
 *   gcc -O2 -Wall -Wextra -o airband_scan pluto_airband_scan_v9.c -liio -lm
 *
 ...
 * v9 note: default run is summary-only every 5 seconds and writes chirp.csv.
 *
 * Example:
...
static long long clamp_to_available_rate(struct iio_channel *chn, long long requested) {
    ...
    if (adjusted != requested && (Modes.verbose || Modes.debug_iio)) {
        fprintf(stderr,
...
static void modesInitPLUTOSDR(void) {
    ...
    if (Modes.verbose || Modes.debug_iio) fprintf(stderr, "* Acquiring IIO context\n");
    ...
    if (Modes.verbose || Modes.debug_iio) fprintf(stderr, "Found %d IIO device(s).\n", device_count);

    if (Modes.verbose || Modes.debug_iio) fprintf(stderr, "* Acquiring AD9361 streaming device cf-ad9361-lpc\n");
    ...
    if (Modes.verbose || Modes.debug_iio) fprintf(stderr, "* Acquiring AD9361 phy device ad9361-phy\n");
    ...
    if (Modes.verbose || Modes.debug_iio) fprintf(stderr, "* Initializing IIO streaming channels voltage0 / voltage1\n");
    ...
    if (Modes.verbose || Modes.debug_iio) fprintf(stderr, "* Creating non-cyclic IIO RX buffer\n");
...
static void emitEvent(const char *event, const struct air_channel *ch,
    ...
    } else if (Modes.console_events && !strcmp(event, "ACTIVE")) {
        if (ch->label[0]) {
...
static void showHelp(void) {
    printf(
        ...
        "Default: summary-only console output every 5 seconds and CHIRP export to chirp.csv.\n"
        "\n"
        "Basic scan options:\n"
        ...
        "  --events                 Print ACTIVE/ENDED transition events to stdout\n"
        "  --json                   Print ACTIVE/ENDED events as JSON lines to stdout\n"
        "  --chirp <file.csv>       Write first ACTIVE per frequency as CHIRP Generic CSV, default chirp.csv\n"
        "  --no-chirp               Disable default CHIRP export\n"
        ...
        "Example:\n"
        "  ./airband_scan\n"
        "  ./airband_scan --events --freq-file phoenix_airband.txt --priority --summary-every 60 --log airband.csv\n"
        "  ./airband_scan --chirp airband_chirp.csv\n"
    );
}
